...
class JobCreateViewTest(APITestCase):
    ...
    def test_create_job_success(self):
        ...
        self.assertEqual(job.title, 'Test Guideline')
        self.assertEqual(job.guideline_text, 'This is a test clinical guideline for diabetes management.')
        self.assertEqual(job.status, 'queued')
    ...
    def test_create_job_missing_guideline_text(self):
        ...
        self.assertEqual(response.data['error'], 'Missing guideline_text')
...
class ProcessGuidelineTaskTest(TestCase):
    ...
    def test_process_guideline_success(self, mock_client):
        ...
        mock_checklist_response.choices[0].message.content = "- Step 1\n- Step 2\n- Step 3"
    ...
    def test_process_guideline_already_processed(self, mock_client):
        ...
        self.assertEqual(result, f"Job {self.event_id} already processed with status: done")
    ...
    def test_process_guideline_checklist_parsing(self, mock_client):
        ...
        mock_checklist_response = MagicMock()
        mock_checklist_response.choices[0].message.content = "• First step\n- Second step\n* Third step"
